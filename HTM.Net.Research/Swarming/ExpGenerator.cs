...
namespace HTM.Net.Research.Swarming
{
    public class ExpGenerator
    {
        ...
        public (ExperimentParameters experimentParameters, ExperimentPermutationParameters permutationParameters) GenerateParams()
        {
            if (Options.streamDef == null) throw new InvalidOperationException("define 'streamDef' for datasource");
            // If the user specified nonTemporalClassification, make sure prediction steps is 0
            int[] predictionSteps = Options.inferenceArgs.predictionSteps;
            ...
            var encoderTuple = _generateEncoderStringsV2(includedFields, Options);
            Map<string, Map<string, object>> encoderSpecs = encoderTuple.Item1.AsMap();
            ...
            return (claParameters, permutationParameters);
        }
            ...
            if (optionInferenceArgs != null)
            {
                ...
                if (options.dynamicPredictionSteps)
                {
                    var altOptionInferenceArgs =
                        JsonConvert.DeserializeObject<InferenceArgsDescription>(JsonConvert.SerializeObject(optionInferenceArgs));
            ...
            foreach (var fieldInfo in includedFields)
            {
                ...
                if (fieldType == FieldMetaType.Float || fieldType == FieldMetaType.Integer)
                {
                    ...
                    else
                    {
                        encoderDict = new EncoderSetting
                        {
                            type = EncoderTypes.AdaptiveScalarEncoder,
                            name = fieldName,
                            fieldName = fieldName,
                            n = 100,
                            w = width,
                    ...
                    if (fieldInfo.minValue.HasValue && fieldInfo.maxValue.HasValue
                        && encoderDict.type == EncoderTypes.AdaptiveScalarEncoder)
                    {
                        encoderDict.type = EncoderTypes.ScalarEncoder;
                    }
        ...
        {
            PermuteEncoder enc = new PermuteEncoder(encoderDict.fieldName, encoderDict.type.ToString(), encoderDict.name);
            if (encoderDict.classifierOnly.GetValueOrDefault(false))
            {
                foreach (string origKey in encoderDict.Keys)
                {
                    ...
                    if (key == "n" && encoderDict.type != EncoderTypes.SDRCategoryEncoder)
                    {
            ...
            {
                // Scalar encoders
                if (new[] { EncoderTypes.ScalarSpaceEncoder, EncoderTypes.AdaptiveScalarEncoder,
                        EncoderTypes.ScalarEncoder, EncoderTypes.LogEncoder}.Contains(encoderDict.type))
                {
                    foreach (string origKey in encoderDict.Keys)
                    {
                        string key = origKey;
                ...
                // Category encoder    
                else if (new[] { EncoderTypes.SDRCategoryEncoder }.Contains(encoderDict.type))
                {
                    foreach (string origKey in encoderDict.Keys)
                    {
                        string key = origKey;
                ...
                // DateTime encoder    
                else if (new[] { EncoderTypes.DateEncoder }.Contains(encoderDict.type))
                {
                    EncoderTypes encoderType = (EncoderTypes)encoderDict["type"];
                    foreach (string origKey in encoderDict.Keys)
                    {
                        string key = origKey;
                        object value = encoderDict[key];
