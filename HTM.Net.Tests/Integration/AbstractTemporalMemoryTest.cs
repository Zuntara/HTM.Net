...
namespace HTM.Net.Tests.Integration
{
     ...
     */
        protected Parameters createTMParams(Parameters overrides)
        {
            ...
            parameters.SetParameterByKey(Parameters.KEY.CELLS_PER_COLUMN, 1);
            parameters.SetParameterByKey(Parameters.KEY.INITIAL_PERMANENCE, 0.8);
            parameters.SetParameterByKey(Parameters.KEY.CONNECTED_PERMANENCE, 0.7);
            parameters.SetParameterByKey(Parameters.KEY.MIN_THRESHOLD, 11);
            parameters.SetParameterByKey(Parameters.KEY.MAX_NEW_SYNAPSE_COUNT, 11);
            parameters.SetParameterByKey(Parameters.KEY.PERMANENCE_INCREMENT, 0.4);
            parameters.SetParameterByKey(Parameters.KEY.PERMANENCE_DECREMENT, 0.0);
            parameters.SetParameterByKey(Parameters.KEY.ACTIVATION_THRESHOLD, 11);
        ...
        public virtual void feedTM(List<HashSet<int>> sequence, String label, bool learn, int num)
        {
            ...
            tm.mmClearHistory();
            foreach (HashSet<int> pattern in repeatedSequence)
            {
                if (pattern.SequenceEqual(SequenceMachine.NONE))
                {
                    tm.ResetSequences(connections);
                }
                else {
                    tm.Compute(connections, pattern.ToArray(), label, learn);
                }
            }
        }
    }
    ...
    [TestClass]
    public class ExtensiveTemporalMemoryTest : AbstractTemporalMemoryTest
    {
        private static readonly PatternMachine PATTERN_MACHINE = new PatternMachine(100, 23, 300, true);
        ...
        public void testB7()
        {
            ...
            Assert.IsTrue(tm.getConnections().GetPermanenceIncrement() == 0.2);
        ...
        public void testB9()
        {
            ...
            Assert.IsTrue(tm.getConnections().GetConnectedPermanence() == 0.7);
            Assert.IsTrue(tm.getConnections().GetPermanenceIncrement() == 0.2);
            Assert.IsTrue(tm.getConnections().GetCellsPerColumn() == 1);
        ...
        public void testB11()
        {
            ...
            Assert.IsTrue(unpredictedActiveColumnsMetric.mean < 1);
        }
        ...
        public void testH1()
        {
            ...
            Assert.IsTrue(predictedInactiveColumnsMetric.mean > 0);

            // At the end of both shared sequences, there should be
            // predicted but inactive columns
            Assert.IsTrue(predictedInactiveTrace.items[14].Count > 0);
            Assert.IsTrue(predictedInactiveTrace.items[33].Count > 0);
        }
        /// <summary>
        ...
        public void testH2()
        {
            ...
            Metric<int> predictedInactiveColumnsMetric = tm.mmGetMetricFromTrace(predictedInactiveTrace);
            Assert.IsTrue(predictedInactiveColumnsMetric.mean > 0);
            Assert.IsTrue(predictedInactiveColumnsMetric.sum < 26);

            // At the end of the second shared sequence, there should be no
            // predicted but inactive columns
            Assert.IsTrue(tm.mmGetTracePredictedInactiveColumns().items[36].Count == 0);
        }
        /// <summary>
        ...
        public void testH3()
        {
            ...
            Metric<int> predictedInactiveColumnsMetric = tm.mmGetMetricFromTrace(predictedInactiveTrace);
            Assert.IsTrue(predictedInactiveColumnsMetric.mean > 0);
            Assert.IsTrue(predictedInactiveColumnsMetric.sum < 26 * 2);

            // At the end of the second shared sequence, there should be
            // predicted but inactive columns
            int i = 0;
            foreach (HashSet<int> s in tm.mmGetTracePredictedInactiveColumns().items)
            {
                Console.WriteLine("" + (i++) + "  " + Arrays.ToString(s));
            }
            Assert.IsTrue(tm.mmGetTracePredictedInactiveColumns().items[4].Count > 0);
            Assert.IsTrue(tm.mmGetTracePredictedInactiveColumns().items[23].Count > 0);
        }
        /// <summary>
        ...
        public void testH4()
        {
            ...
            IndicesTrace predictedInactiveTrace = tm.mmGetTracePredictedInactiveColumns();
            Metric< int> predictedInactiveColumnsMetric = tm.mmGetMetricFromTrace(predictedInactiveTrace);
            Assert.IsTrue(predictedInactiveColumnsMetric.mean < 3);
        }
        /// <summary>
        ...
        public void testH10()
        {
            ...
            List<List<HashSet<int>>> sequenceNoisy = new List<List<HashSet<int>>>();
            for (int i = 0; i < 10; i++)
            {
            ...
            Assert.IsTrue(predictedInactiveColumnsMean1 > 0);
        ...
        public override void feedTM(List<HashSet<int>> sequence, String label, bool learn, int num)
        {
            ...
            if (VERBOSITY >= 2)
            {
                Console.WriteLine(tm.mmPrettyPrintTraces(
            ...
            if (learn && VERBOSITY >= 3)
            {
                ...
                //Console.WriteLine(tm.mmPrettyPrintConnections());
            }
        }
        ...
        private List<HashSet<int>> translateSequence(string s)
        {
            ...
            string[] sa = Regex.Split(s, "[\\s]*\\)\\,[\\s]*");
            foreach (string numList in sa)
            {
                //string[] nums = numList.split("[\\s]*\\,[\\s]*");
                string[] nums = Regex.Split(numList, "[\\s]*\\,[\\s]*");
                HashSet<int> set = new HashSet<int>(nums.Select(
                    n => n.Equals("None") ? -1 : int.Parse(n)).ToList());
                retVal.Add(set);
            }

            List<HashSet<int>> copy = new List<HashSet<int>>(retVal);
            for (int i = 0; i < copy.Count; i++)
            {
                HashSet<int> set = copy[i];
                if (set.Contains(-1))
                {
                    retVal[i].Clear();
                    retVal[i] = SequenceMachine.NONE;
                }
            }
        ...
        public void assertAllActiveWerePredicted()
        {
            ...
            Metric<int> predictedActiveColumnsMetric = tm.mmGetMetricFromTrace(tm.mmGetTracePredictedActiveColumns());

            Assert.AreEqual(unpredictedActiveColumnsMetric.sum, 0, 0);
            Assert.AreEqual(predictedActiveColumnsMetric.min, 21, 0);
        ...
        public void assertAllActiveWereUnpredicted()
        {
            Metric<int> unpredictedActiveColumnsMetric = tm.mmGetMetricFromTrace(
                tm.mmGetTraceUnpredictedActiveColumns());
            Metric<int> predictedActiveColumnsMetric = tm.mmGetMetricFromTrace(
                tm.mmGetTracePredictedActiveColumns());

            Assert.AreEqual(predictedActiveColumnsMetric.sum, 0, 0);
            Assert.AreEqual(unpredictedActiveColumnsMetric.min, 21, 0);
            Assert.AreEqual(unpredictedActiveColumnsMetric.max, 25, 0);
        }
    }
}