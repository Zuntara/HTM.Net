...
public class NetworkApiDemoTests
{
    ...
    public void TestGuesses()
    {
        ...
        correctGuesses = guess.Item1;
        correctBonus = guess.Item2;
        Assert.AreEqual(4, correctGuesses);
        Assert.AreEqual(false, correctBonus);
    ...
    public void SimulateBasedOnStatistics()
    {
        ...
        var statsData = allData.Skip(testCount).ToList();
        var testData = allData.Take(testCount).ToList();

        List<double[]> stats = new List<double[]>();

        stats.Add(RandomGuessNetworkApi.GetBestChance(50, 0, statsData).Select(d => (double)d).ToArray());
        stats.Add(RandomGuessNetworkApi.GetBestChance(100, 0, statsData).Select(d => (double)d).ToArray());
        //stats.Add(RandomGuessNetworkApi.GetBestChance(250, 0, statsData).Select(d => (double)d).ToArray());
        //stats.Add(RandomGuessNetworkApi.GetBestChance(500, 0, statsData).Select(d => (double)d).ToArray());
        //stats.Add(RandomGuessNetworkApi.GetBestChance(1000, 0, statsData).Select(d => (double)d).ToArray());
        //stats.Add(RandomGuessNetworkApi.GetBestChance(2000, 0, statsData).Select(d => (double)d).ToArray());
        stats.Add(RandomGuessNetworkApi.GetBestChance(statsData.Count, 0, statsData).Select(d => (double)d).ToArray());
    ...
    public void RunBasicNetwork()
    {
        ...
        // 3300
        RandomGuessNetworkApi demo = new RandomGuessNetworkApi(trainCount, testCount, 0, false);
        demo.RunNetwork();

        var allGuessesObjs = demo.GetGuesses().ToList();
        var trainedGuessObjs = RandomGuess.GetFirstGuesses(allGuessesObjs, trainCount);
        var lastGuessesObjs = RandomGuess.GetLastGuesses(allGuessesObjs, testCount);
        var lastGuessObjs = RandomGuess.GetLastGuesses(allGuessesObjs, 1);

        PrintHistogramAndProfits(trainedGuessObjs);
        ...
        Console.WriteLine("Next predictions:");
        int p = 1;
        foreach (var prediction in lastGuessObjs.FirstOrDefault()?.NextPredictions ?? new List<int[]>())
        {
            Console.WriteLine($"{p++} - {Arrays.ToString(prediction)}");
        }
