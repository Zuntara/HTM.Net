 ...
 */
    public class NetworkDemoHarness
    {
        ...
        public static Map<string, Map<string, object>> GetHotGymFieldEncodingMap()
        {
            ...
            fieldEncodings["timestamp"].Add(Parameters.KEY.DATEFIELD_DOFW.GetFieldName(), new BitsTuple(21, 24.0)); // Day of week
            fieldEncodings["timestamp"].Add(Parameters.KEY.DATEFIELD_TOFD.GetFieldName(), new BitsTuple(21, 4.0)); // Time of day
        ...
        public static Map<string, Map<string, object>> GetNetworkDemoFieldEncodingMap()
        {
            ...
            fieldEncodings = SetupMap(
                    ...
                    21,
                    0, 30, 
                    0, 0.5, null, true, null,
                    "consumption", "float", "AdaptiveScalarEncoder");

            fieldEncodings["timestamp"].Add(Parameters.KEY.DATEFIELD_TOFD.GetFieldName(), new BitsTuple(11, 2.0)); // Time of day
            fieldEncodings["timestamp"].Add(Parameters.KEY.DATEFIELD_DOFW.GetFieldName(), new BitsTuple(11, 1.0)); // Day of week
            fieldEncodings["timestamp"].Add(Parameters.KEY.DATEFIELD_PATTERN.GetFieldName(), "YYYY-MM-dd HH:mm");
        ...
        public static Parameters GetNetworkDemoTestEncoderParams()
        {
            ...
            p.SetParameterByKey(Parameters.KEY.CELLS_PER_COLUMN, 16);
            p.SetParameterByKey(Parameters.KEY.NUM_ACTIVE_COLUMNS_PER_INH_AREA, 40.0);

            //p.setParameterByKey(KEY.POTENTIAL_PCT, 0.8);
            p.SetParameterByKey(Parameters.KEY.POTENTIAL_PCT, 0.5);
            p.SetParameterByKey(Parameters.KEY.SYN_PERM_CONNECTED, 0.21);
            //p.setParameterByKey(KEY.SYN_PERM_ACTIVE_INC, 0.0001);
            //p.setParameterByKey(KEY.SYN_PERM_INACTIVE_DEC, 0.0005);
            p.SetParameterByKey(Parameters.KEY.SYN_PERM_ACTIVE_INC, 0.07); // 0.07
            p.SetParameterByKey(Parameters.KEY.SYN_PERM_INACTIVE_DEC, 0.01); // 0.01
                                                                  //p.setParameterByKey(KEY.MAX_BOOST, 1.0);
            p.SetParameterByKey(Parameters.KEY.MAX_BOOST, 1.05);
            ...
            p.SetParameterByKey(Parameters.KEY.PERMANENCE_INCREMENT, 0.07);
            p.SetParameterByKey(Parameters.KEY.PERMANENCE_DECREMENT, 0.01);
            //p.setParameterByKey(KEY.MIN_THRESHOLD, 18);
            p.SetParameterByKey(Parameters.KEY.MIN_THRESHOLD, 22); // 22
            p.SetParameterByKey(Parameters.KEY.ACTIVATION_THRESHOLD, 16); // 16
        ...
        public static Parameters GetParameters()
        {
            ...
            parameters.SetParameterByKey(Parameters.KEY.MAX_NEW_SYNAPSE_COUNT, 20);
